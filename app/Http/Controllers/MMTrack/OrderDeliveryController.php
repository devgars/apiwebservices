...
class OrderDeliveryController extends Controller
{
    ...
    public function cambiasEstatus(Request $request)
    {
        ...
        switch ($request->estatus) {
            ...
            case 'Punto de Destino':
                $modificaciones_tracking = 33;
                $modificaciones = ['status_order' => $request->estatus, 'arrival_date'=>date('Y-m-d H:i:s') ];
                $modificaciones_tracking_text = "Pedido Llego al Punto de Entrega";
        ...
        if ($modificaciones_tracking>0) {
            ...
            for ($i=0;  $i<=count($request->pedidos)-1 ; $i++) { 
                ...
                $datos_pedido = Dispatch::select('document_order_number','customer_code','customer_id','social_reason','customer_contact_id')->where('id',$request->pedidos[$i])->first();
                ...
                DB::table('log_mensajes')->insert([
                    'cliente_id' => $datos_pedido->customer_id,
                    'mensaje' => $log_mensaj,
                    'pedido_id' => $request->pedidos[$i],
                    'tipo' => "WS",
                    'transito_id' => $modificaciones_tracking_text
                ]);
            ...
            foreach ($envios as $envio) {
                ...
                OrderDeliveryDetail::where('order_delivery_id',$request->order_delivery_id)
                    ...
                    ->update([  'tipo_entrega' =>$request->tipo_entrega, 
                                'agency_date' => $request->fecha." ".$request->hora, 
                ...
                $datos_pedido = Dispatch::select('document_order_number','customer_code','customer_id','social_reason','customer_contact_id')->where('id',$envio->dispatch)->first();
                ...
                DB::table('log_mensajes')->insert([
                    'cliente_id' => $datos_pedido->customer_id,
                    'mensaje' => $log_mensaj,
                    'pedido_id' => $envio->dispatch,
                    'tipo' => "WS",
                    'transito_id' => $ws_mensaje_transito
                ]);
                ...
                DispatchDetailsTracking::Create([
                    ...
                    'date_of_work' => $request->fecha,
                    'work_time' =>  $request->hora,
                    'status' => 1,
                ]);
    ...
    */
    ...
    public function obtenerFotos($id)
    {
        ...
        $fotos = OrderDeliveryDetail::select('imagen','imagen_bultos','image_64_bultos','image_64')->where('id',$id)->first();
       
        $url_imagenes = Config::get('services.servidor_imagenes.url');

        $foto_peso = "$url_imagenes$fotos->imagen";
        $foto_bulto = "$url_imagenes$fotos->imagen_bultos";

        if (!$fotos->imagen && $fotos->image_64) {
            $foto_peso = substr($fotos->image_64, 0,5)=='data:' ? $fotos->image_64 : "data:image/jpeg;base64,$fotos->image_64";
        }

        if (!$fotos->imagen_bultos && $fotos->image_64_bultos) {
        ...
        if($fotos->imagen || $fotos->image_64){
            array_push($imagenes,[  "original" => $foto_peso, "thumbnail" => $foto_peso, ]);
        }

        if ($fotos->imagen_bultos || $fotos->image_64_bultos) {
            array_push($imagenes,[  "original" => $foto_bulto, "thumbnail" => $foto_bulto, ]);
        }
    ...
    public function guardarImagen(Request $request)
    {
        ...
        foreach ($pedidos as $pedido) {
            ...
            //crear directorio
            Storage::makeDirectory("public/$nombre_carpeta");

            //ruta de la imagen
            $ruta_mysql = "/storage/$nombre_carpeta/$nombre_imagen";
            $ruta = storage_path()."/app/public/$nombre_carpeta/$nombre_imagen";
            ...
            if ($tipo == 'peso') {
                OrderDeliveryDetail::where('id',$pedido->id)->update(['imagen' => $ruta_mysql]);
            }else{
                OrderDeliveryDetail::where('id',$pedido->id)->update(['imagen_bultos' => $ruta_mysql]);
            }
        }
